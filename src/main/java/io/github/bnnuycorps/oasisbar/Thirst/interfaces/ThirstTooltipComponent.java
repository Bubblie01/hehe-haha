...
public class ThirstTooltipComponent implements TooltipComponent {
    ...
    public void drawItems(TextRenderer textRenderer, int x, int y, DrawContext context) {
        for (int i = 0; i < this.thirstQuench / 2; i++) {
            context.drawTexture(RenderInit.DEHYDRATION_ICON, x + i * 9 - 1, y, 0, 0, 9, 9, 256, 256); // Background
            context.drawTexture(RenderInit.DEHYDRATION_ICON, x + i * 9 - 1, y, this.quality * 18, 9, 9, 9, 256, 256);
        }
        if (this.thirstQuench % 2 != 0) {
            context.drawTexture(RenderInit.DEHYDRATION_ICON, x + this.thirstQuench / 2 * 9 - 1, y, 0, 0, 9, 9, 256, 256); // Background
            context.drawTexture(RenderInit.DEHYDRATION_ICON, x + this.thirstQuench / 2 * 9 - 1, y, this.quality * 18 + 9, 9, 9, 9, 256, 256);
        }
    }
